...
class GrowthRules:
    ...
    def __init__(self):
        ...
        self.growth_rules = [
            GrowthRule("add_node", cost=10.0, probability=0.4, min_gp=10.0, max_nodes=50),
            GrowthRule("split_node", cost=15.0, probability=0.3, min_gp=15.0, max_nodes=40),
            GrowthRule("add_connection", cost=5.0, probability=0.3, min_gp=5.0, max_nodes=100),
        ]
    ...
    def select_growth_type(self, brain: 'Brain') -> Optional[str]:
        """
        ...
        available_rules = [
            rule for rule in self.growth_rules
            if (brain.gp >= rule.min_gp and 
                brain.phenotype.num_nodes < rule.max_nodes)
        ]
    ...
    def _can_grow(self, brain: 'Brain') -> bool:
        """
    ...
    def _get_complexity_score(self, brain: 'Brain') -> float:
        """
        ...
        # Взвешенная сумма
        complexity = (0.4 * node_complexity + 
                     0.3 * density_complexity + 
                     0.3 * path_complexity)
        
        return complexity
    
    def apply_growth_penalty(self, brain: 'Brain'):
        """
    ...
    def can_apply_growth(self, brain: 'Brain', growth_type: str) -> bool:
        """
        ...
        for rule in self.growth_rules:
            if rule.name == growth_type:
                return (brain.gp >= rule.min_gp and 
                        brain.phenotype.num_nodes < rule.max_nodes)
        return False
    
    def get_available_growth_types(self, brain: 'Brain') -> List[str]:
        """
    ...
    def optimize_growth_strategy(self, brain: 'Brain') -> Optional[str]:
        """
    ...
    def remove_growth_rule(self, rule_name: str):
        ...
        """
        self.growth_rules = [rule for rule in self.growth_rules if rule.name != rule_name]
    ...
    def get_growth_statistics(self, brain: 'Brain') -> Dict[str, Any]:
        """
    ...
    def __repr__(self):
        return (f"GrowthRules(rules={len(self.growth_rules)}, "
