...
class Genome:
    ...
    def __init__(self, input_size: int = 2, output_size: int = 1, hidden_size: int = 3):
    ...
    def add_node(
        self, node_type: str = "hidden", activation_function: str = "sigmoid"
    ) -> int:
        """
    ...
    def clone(self) -> "Genome":
        ...
        new_genome.connection_genes = [
            ConnectionGene(**vars(conn)) for conn in self.connection_genes
        ]
    ...
    def __repr__(self):
        return (
            ...
            f"connections={len(self.connection_genes)})"
        )
