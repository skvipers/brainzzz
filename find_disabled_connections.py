...
def check_brain_connections(
    brain_id: int, api_url: str = "http://localhost:8000"
) -> Dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            disabled_connections = len(
                [c for c in connections if not c.get("enabled", True)]
            )

            disabled_list = [
                {"id": c["id"], "from": c["from"], "to": c["to"], "weight": c["weight"]}
        ...
        return {"brain_id": brain_id, "status": "error", "error": str(e)}


def find_brains_with_disabled_connections(
    brain_range: range = range(1, 21), api_url: str = "http://localhost:8000"
):
