...
class BrainVisualizer:
    ...
    def visualize_brain(self, brain: Brain, 
                       save_path: Optional[str] = None,
                       show_activations: bool = True,
                       show_weights: bool = True) -> plt.Figure:
        """
        ...
        # Заголовок
        fig.suptitle(f'Визуализация мозга\n'
    ...
    def _plot_structure_graph(self, G: nx.DiGraph, brain: Brain, ax: plt.Axes, show_weights: bool):
        """Рисует граф структуры."""
        # Позиции узлов
        pos = self._calculate_node_positions(brain)
        
        # Цвета узлов
        node_colors = [self.colors.get(G.nodes[node]['type'], '#757575') 
                      for node in G.nodes()]
        
        # Размеры узлов
        node_sizes = [300 if G.nodes[node]['type'] in ['input', 'output'] else 200 
                     for node in G.nodes()]
    ...
    def _plot_activation_graph(self, G: nx.DiGraph, brain: Brain, ax: plt.Axes):
        ...
        if not brain.history:
            ax.text(0.5, 0.5, 'Нет данных активации', 
        ...
        # Подписи узлов
        labels = {node: f"{node}\n{activations[node]:.2f}" 
                 if node < len(activations) else f"{node}\n0.00"
                 for node in G.nodes()}
        nx.draw_networkx_labels(G, pos, labels, font_size=8, ax=ax)
    ...
    def plot_thought_path(self, brain: Brain, save_path: Optional[str] = None) -> plt.Figure:
        """
        ...
        if not thought_path:
            ...
            ax.text(0.5, 0.5, 'Нет данных о пути мысли', 
                   ha='center', va='center', transform=ax.transAxes)
        ...
        if brain.history:
            ...
            for node_id in range(min(activations.shape[1], 10)):  # Первые 10 узлов
                ax1.plot(time_steps, activations[:, node_id], 
                        label=f'Узел {node_id}', alpha=0.7)
        ...
        # Путь мысли
        ax2.plot(range(len(thought_path)), thought_path, 'o-', linewidth=2, markersize=8)
        ax2.set_xlabel('Шаг')
