...
class Mutation:
    ...
    def _mutate_growth_rules(self, brain: Brain):
        ...
        if random.random() < self.mutation_strength:
            ...
            growth_rules.growth_probability = max(0.01, min(0.5, growth_rules.growth_probability))
        
        if random.random() < self.mutation_strength:
            growth_rules.complexity_penalty += random.uniform(-0.005, 0.005)
            growth_rules.complexity_penalty = max(0.001, min(0.1, growth_rules.complexity_penalty))
