...
class Crossover:
    ...
    def _uniform_crossover(self, parent1: Brain, parent2: Brain) -> Brain:
        ...
        # Скрещиваем геномы
        offspring.genome = self._crossover_genomes(parent1.genome, parent2.genome, 'uniform')
        
        # Скрещиваем правила роста
        offspring.growth_rules = self._crossover_growth_rules(parent1.growth_rules, parent2.growth_rules)
    ...
    def _single_point_crossover(self, parent1: Brain, parent2: Brain) -> Brain:
        ...
        offspring = parent1.clone()
        offspring.genome = self._crossover_genomes(parent1.genome, parent2.genome, 'single_point')
        offspring.growth_rules = self._crossover_growth_rules(parent1.growth_rules, parent2.growth_rules)
        offspring.phenotype = offspring.phenotype.__class__(offspring.genome)
        return offspring
    ...
    def _two_point_crossover(self, parent1: Brain, parent2: Brain) -> Brain:
        ...
        offspring = parent1.clone()
        offspring.genome = self._crossover_genomes(parent1.genome, parent2.genome, 'two_point')
        offspring.growth_rules = self._crossover_growth_rules(parent1.growth_rules, parent2.growth_rules)
        offspring.phenotype = offspring.phenotype.__class__(offspring.genome)
        return offspring
    ...
    def _crossover_genomes(self, genome1, genome2, method: str):
        ...
        if method == 'uniform':
            # Равномерное скрещивание: каждый ген с вероятностью 0.5 берётся от одного из родителей
            for i, node in enumerate(new_genome.node_genes):
                if random.random() < 0.5:
                    parent2_node = genome2.node_genes[i] if i < len(genome2.node_genes) else node
            ...
            for i, conn in enumerate(new_genome.connection_genes):
                if random.random() < 0.5:
                    parent2_conn = genome2.connection_genes[i] if i < len(genome2.connection_genes) else conn
                    conn.weight = parent2_conn.weight
                    conn.plasticity = parent2_conn.plasticity
        
        elif method == 'single_point':
            # Скрещивание в одной точке
            crossover_point = random.randint(0, len(new_genome.node_genes) // 2)
            ...
            # Скрещивание в двух точках
            point1 = random.randint(0, len(new_genome.node_genes) // 3)
            point2 = random.randint(2 * len(new_genome.node_genes) // 3, len(new_genome.node_genes))
    ...
    def _crossover_growth_rules(self, rules1, rules2):
        ...
        # Клонируем правила первого родителя
        new_rules = rules1.__class__()
    ...
    def set_crossover_type(self, crossover_type: str):
        ...
        valid_types = ['uniform', 'single_point', 'two_point']
        if crossover_type in valid_types:
            self.crossover_type = crossover_type
        else:
            raise ValueError(f"Неизвестный тип скрещивания: {crossover_type}. Доступные: {valid_types}")
